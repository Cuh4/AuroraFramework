...
AuroraFramework.services.playerService.setDedicatedServer(true) -- Cannot be undone. This effectively makes it so the host player (player with peer ID 0) gets removed internally. In dedicated servers, the host player is the server itself.

-- Get players
local host = AuroraFramework.services.playerService.getPlayerByPeerID(0) -- Host player (the one who created the server, or the first player to join a dedicated server)
